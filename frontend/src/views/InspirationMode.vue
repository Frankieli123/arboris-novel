    ...
    const { startPolling, stopPolling } = useTaskPolling({
      ...
      onComplete: async (result) => {
        ...
        if (result && result.ai_message) {
          ...
          if (result.is_complete && result.ready_for_blueprint) {
            // 对话完成，显示蓝图确认界面
            confirmationMessage.value = result.ai_message
            showBlueprintConfirmation.value = true
            ...
            // 继续对话
            currentUIControl.value = result.ui_control
          }
          
          // 更新conversation state
          if (result.conversation_state) {
            novelStore.currentConversationState = result.conversation_state
          }
        }
      },
      onError: (error) => {
        ...
        globalAlert.showError(`对话处理失败: ${error}`, '任务失败')
        // 不重置整个模式，让用户可以重试
      }
    })
  ...
  try {
    ...
    const { startPolling, stopPolling } = useTaskPolling({
      ...
      onComplete: async (result) => {
        ...
        // 处理蓝图生成结果
        if (result) {
          handleBlueprintGenerated(result)
        }
      },
      onError: (error) => {
        stopPolling()
