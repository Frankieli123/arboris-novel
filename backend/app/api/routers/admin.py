...
async def get_task_statistics(
    ...
    Args:
        time_window_hours: 时间窗口（小时），用于性能指标统计
        
    Returns:
        任务统计信息，包括各状态任务数、按类型分组的任务数和性能指标
    """
    from ...services.task_service import TaskService
    from ...services.task_metrics_service import TaskMetricsService
    from ...schemas.task import TaskStatistics
    ...
    # 获取性能指标
    try:
        execution_metrics = await metrics_service.get_task_execution_metrics(
            time_window_hours=time_window_hours
        )
        success_metrics = await metrics_service.get_task_success_rate(
            time_window_hours=time_window_hours
        )
        waiting_metrics = await metrics_service.get_task_waiting_time_metrics(
            time_window_hours=time_window_hours
        )
        
        avg_execution_time = execution_metrics.get("avg_execution_time_seconds", 0)
        success_rate = success_metrics.get("success_rate_percent", 0)
        avg_waiting_time = waiting_metrics.get("avg_waiting_time_seconds", 0)
    except Exception as e:
        logger.warning("获取性能指标失败: %s", str(e))
        avg_execution_time = 0
        success_rate = 0
        avg_waiting_time = 0
    
    logger.info("管理员查询任务统计信息")
    ...
    return TaskStatistics(
        ...
        tasks_by_type=stats["tasks_by_type"],
...
async def list_all_tasks(
    ...
    """
    from ...services.task_service import TaskService
    from ...schemas.task import AdminTaskSummary
    ...
    logger.info(
        "管理员查询任务列表: status=%s, task_type=%s, limit=%s, offset=%s, 返回 %s 条",
        status, task_type, limit, offset, len(tasks)
    )
    
    return [AdminTaskSummary.model_validate(task) for task in tasks]
